...
from src.constants import NTEP_COLOR_SCALE
...
def transform_layout_to_long_format(layout: pd.DataFrame, value_name: str) \
        -> pd.DataFrame:
    """
    ...
    layout = layout.reset_index().melt(id_vars='ROW', var_name='COL',
...
def get_model_data(data, **kwargs):
    ...
    # T = max(tt)
    # K = max(kk)
    model_data = {'y': y, 'ii': ii, 'jj': jj, 'pp': pp,
                  'N': N, 'M': M, 'I': I, 'J': J, 'P': P, }
...
def plot_ratings_over_time(data, plot_dims=[5.5, 5.5]):
    ...
    """
    dfs = [data[data.DATE == date].reset_index(
        drop=True) for date in data.DATE.unique()]
    # generate the frames. NB name
    frames = [
        go.Frame(
            data=go.Heatmap(
                z=df.QUALITY, y=df.ROW, x=df.COL, hoverinfo='text',
                text=['Entry:' + name + '<br />Quality:' +
                      str(quality) for name, quality in zip(df.ENTRY_NAME, df.QUALITY)],
    ...
    fig.update_layout(
        ...
        xaxis={"title": 'Col #', "tickangle": 0,
               'side': 'top',  "tick0": 1, "dtick": 1},
        title_x=0.5
    )
    return fig


def plot_field_heat_map(plt_effect, row_id, col_id, entry_name, fig_width=400, fig_height=600):
    fig6 = go.Figure()
    fig6.add_traces(go.Heatmap(z=plt_effect, y=row_id, x=col_id, hoverinfo='text',
...
def calc_dist_matrix(layout_data, index_col='PLOC_CODE',
                     row_len=5.5, col_len=5.5,
                     row_gap=0, col_gap=0):
    """
...
def select_cultivar_name(name_list):
    ...
    name_list_new = [
        x for x in name_list if shortest_name.lower() in x.lower()]
...
def extract(variable, results, n_burnin=400):
    ...
    variable_data = pd.DataFrame(results_data.loc[:, (results_data.columns.str.startswith(variable)) &
                                                  (~results_data.columns.str.contains('free'))].mean(),
                                 columns=[col_name])
    try:
        variable_data[var_name] = variable_data.index.str.replace(
            str_to_drop, '').astype(int)
    except TypeError:
...
def plot_thresholds(kappa):
    ...
    fig.add_trace(go.Scatter(
        x=kappa, y=[0]*8, mode='markers', marker_size=15,
        hoverinfo='x'
    ))
...
def plot_effect_size(results_data):
    var_data = results_data.loc[:, (~results_data.columns.str.contains('free'))
                                ].filter(regex='year|month|entry|beta|plot').mean().reset_index()
    var_data[['VAR', 'CODE']] = var_data['index'].str.split('.', expand=True)
    var_data.rename(columns={0: 'VALUE'}, inplace=True)
    fig = px.box(var_data, y='VAR', x='VALUE',
                 category_orders={
                     'VAR': ['month', 'year', 'plot', 'entry', 'beta']},
                 labels={'VALUE': 'Effect Size on Logit Scale', 'VAR': ''})
    fig.update_layout(
        yaxis=dict(
            tickmode='array',
            tickvals=['plot', 'beta', 'month', 'year', 'entry'],
            ticktext=['Plot Location Effect', 'Rating Severity',
                      'Month Effect', 'Year Effect', 'Entry Quality']
        )
    )
    fig.show()
