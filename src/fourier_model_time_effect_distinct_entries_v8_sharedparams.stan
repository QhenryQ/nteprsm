...
// cross_validation
//  array[N] int <lower=0, upper=1> test_set;
...
transformed data {
  ...
  for (n in 1:N) {
	xn[entry_cumcount[n]] = (time[n]-mean_time)/sd_time;
  }
...
parameters {
  ...
  real<lower=0> sigma_f;       		// scale(variance) of f
...
transformed parameters {
  ...
  matrix[num_ratings_per_entry ,2*M_f] PHI_f;
  ...
  for(i in 1:num_entries) {
	time_effect[i] = intercept_f[i] + PHI_f * (diagSPD_f .* beta_f[i]); //+ sigma_time*z_time[i];
  }
...
model {
  ...
  alpha_plot ~ normal(0,4	);
  to_vector(z) ~ std_normal();
  lengthscale_plot ~ inv_gamma(5,2);
  ...
  lengthscale_f ~ inv_gamma(5,2);	// Gaussian Process lengthscale parameter
  sigma_f ~ normal(0,4);			// Gaussian Process variance parameter
  for (i in 1:num_raters) {
...
generated quantities {
	...
	for (n in 1:N) log_lik[n] = rsm(y[n], plot_effect[plot_id[n]]+time_effect[entry_id[n]][entry_cumcount[n]], tau_rsm[rater_id[n]]);
}