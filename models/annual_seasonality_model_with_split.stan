...
data {  
  ...
  int<lower=1> y_max;                   // the max value of y when indexed from 0, which is also number of categories.
  ...
  array[N] int<lower=1, upper=num_plots> plot_code; // use plot_code, as plot id has been used at each trial
...
transformed data {
  ...
  // pred_xn is the standardized array of prediction times for the time effect GP
  ...
  // standardize test times using training mean/sd and build test basis matrix
...
generated quantities {
  ...
  // test set predictions and log-likelihood
  for (i in 1:num_entries)
    time_effect_test[i] = intercept_f[i] + PHI_f_test * (diagSPD_f .* beta_f[i]);
  for (n in 1:N_test) {
    real theta_test_n = plot_effect[plot_code_test[n]]
                        + time_effect_test[entry_code_test[n]][rating_event_code_test[n]];
    vector[y_max + 1] probs_test = softmax(
      cumulative_sum(append_row(rep_vector(0, 1), theta_test_n - tau_rater[rater_code_test[n]]))
    );
