...
transformed parameters {
  ...
  for (j in 1:num_entries)
    for (k in 1:(2*M_f))
	  {
/* 		print("L: ", L, ", num_entries: ", num_entries, ", 2*M_f: ", 2*M_f);
		print("size(L_cov): ", dims(L_cov));
		print("size(z_beta): ", dims(z_beta));
		print("size(z_beta[k]): ", dims(z_beta[k]));
		print("size(z_beta[k][, j]): ", dims(col(z_beta[k], j))); // safer than z_beta[k][, j]
		// print("resulting matrix size: ", dims(L_cov * z_beta[k][, j]));
		// print("resulting matrix-vector size: ", dims(L_cov * col(z_beta[k],j))); */
...
model {
  ...
  // priors on beta 
  for (i in 1:num_raters) target += normal_lpdf(beta_rater[i] | 0, 2);
